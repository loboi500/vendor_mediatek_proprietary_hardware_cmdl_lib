...
// TV secure issue: import FD memory and copy memory by GPU
MKErrorType MKProgram_AttachCLImportMemory(
    MKProgram mk_program,
    cl_context context,
    cl_mem_flags flags,
...
MKErrorType MKProgram_AttachCLPrepareMemoryCopy(
    MKProgram mk_program,
    cl_context context,
    cl_uint num_devices,
    const cl_device_id *devices,
...
MKErrorType MKProgram_AttachCLEnqueueMemoryCopy(
    MKProgram mk_program,
    cl_command_queue cmdQueue,
    cl_kernel kernel,
    cl_bool bBlocking,
    ...
    cl_mem srcMem,
    uint32_t copyLength,
    uint32_t dstStride[4],
    uint32_t srcStride[4],
    uint32_t dimension[4]);

MKErrorType MKProgram_ReplaceWithCLCreateProgramWithBinary(
    MKProgram mk_program);
